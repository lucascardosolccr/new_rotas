...
def _contar_rotas_geo_validas(df):
    ...
    iterrows): mesma regra (ao menos UMA das 4 coordenadas ≠ 0), sem laço linha a linha."""
    _cols = ['Lat Origem', 'Lon Origem', 'Lat Destino', 'Lon Destino']
    _presentes = [c for c in _cols if c in df.columns]
    if not _presentes:
        return 0
    try:
        _num = df[_presentes].apply(pd.to_numeric, errors='coerce').fillna(0.0)
        return int((_num != 0).any(axis=1).sum())
    except Exception:
        return 0
...
def _auditoria_validacao_ibge(texto_usuario, cod_identificado, municipio, uf, fonte):
    """[IBGE-INPUT - 99ª geração] Monta a auditoria 'Validação Oficial pelo Código IBGE' de um ponto:
    compara o código INFORMADO pelo usuário (se digitou um) com o código IDENTIFICADO, e reporta a
    consistência (confirmado / divergência / identificado por geocodificação). PURA. Depende de
    _e_codigo_ibge para detectar se a entrada foi um código."""
    _cod_informado = _e_codigo_ibge(texto_usuario) if "_e_codigo_ibge" in globals() else ""
    _cod_ident = str(cod_identificado or "").strip()
    _cod_ident = "" if _cod_ident in ("—", "N/A", "0", "None", "") else _cod_ident
    _por_codigo = bool(_cod_informado)
    if _por_codigo:
        if _cod_informado == _cod_ident:
            _status, _rev = "✅ Confirmado", "Não"
            _diverg = "Nenhuma — o código informado corresponde ao município identificado."
        elif _cod_ident:
            _status, _rev = "⚠️ Divergência", "Sim"
            _diverg = (f"O código informado ({_cod_informado}) difere do identificado ({_cod_ident}). "
                       f"Correção sugerida: usar {_cod_ident} ({municipio}/{uf}).")
        else:
            _status, _rev = "⚠️ Não confirmado", "Sim"
            _diverg = f"O código informado ({_cod_informado}) não pôde ser confirmado na base oficial."
    else:
        _status, _rev = "ℹ️ Identificado por geocodificação", "Não"
        _diverg = "A entrada não foi um Código IBGE; o código foi obtido pela identificação do município."
    return {
        "entrada_por_codigo": _por_codigo,
        "codigo_informado": _cod_informado or "—", "codigo_identificado": _cod_ident or "—",
        "municipio": municipio or "—", "uf": uf or "—", "fonte": fonte or "—",
        "status": _status, "divergencia": _diverg, "revisao_manual": _rev,
    }
    ...
    if _cod_in:
        ...
        if _res_cod and (_res_cod["lat"] or _res_cod["lon"]):
            ...
            return (float(_res_cod["lat"]), float(_res_cod["lon"]),
                    ...
                    [f"🔒 Rota TRAVADA na identidade oficial do IBGE. Entrada reconhecida como Código IBGE "
...
# ==============================================================================
# [IBGE-MALHAS - 108ª geração] VALIDAÇÃO POR POLÍGONO OFICIAL (API de malhas do IBGE) — opcional/gated.
# Verifica se a coordenada geocodificada cai DENTRO do polígono oficial do município (Código IBGE),
# detectando erros de geocodificação (ponto caiu no município vizinho, etc.). O núcleo (point-in-polygon
# por ray casting) é PURO e testável, SEM dependências externas; o download do polígono (API v3 de malhas
# do IBGE, GeoJSON leve/cacheado) é abstraído e DESLIGADO por padrão (sem rede no ambiente).
# ==============================================================================
IBGE_MALHAS_ATIVO = False  # mestra: ativa a validação por polígono (requer rede à API de malhas do IBGE)


def _extrair_aneis_geojson(geometry):
    """[IBGE-MALHAS - 108ª geração] Achata uma geometria GeoJSON (Polygon/MultiPolygon) numa lista de
    anéis [[lon,lat],...]. PURO. Retorna [] em qualquer inconsistência."""
    if not isinstance(geometry, dict):
        return []
    _t = geometry.get("type")
    _c = geometry.get("coordinates")
    _aneis = []
    try:
        if _t == "Polygon":
            for _anel in _c:
                _aneis.append(_anel)
        elif _t == "MultiPolygon":
            for _poly in _c:
                for _anel in _poly:
                    _aneis.append(_anel)
    except Exception:
...
def _ponto_em_poligono(lat, lon, aneis):
    """[IBGE-MALHAS - 108ª geração] Point-in-polygon por RAY CASTING. `aneis` = lista de anéis, cada anel
    uma lista de pares [lon, lat] (padrão GeoJSON). Dentro se o ponto estiver dentro de um número ÍMPAR de
    anéis (trata buracos de Polygon e partes de MultiPolygon). PURO, sem dependências. Retorna bool."""
    try:
        _x, _y = float(lon), float(lat)
    except (TypeError, ValueError):
        return False
    _dentro = False
    for _anel in aneis or []:
        _n = len(_anel)
        if _n < 3:
            continue
        _j = _n - 1
        for _i in range(_n):
            try:
                _xi, _yi = _anel[_i][0], _anel[_i][1]
                _xj, _yj = _anel[_j][0], _anel[_j][1]
            except (IndexError, TypeError):
                _j = _i
                continue
            if ((_yi > _y) != (_yj > _y)) and \
               (_x < (_xj - _xi) * (_y - _yi) / ((_yj - _yi) if (_yj - _yi) != 0 else 1e-12) + _xi):
                _dentro = not _dentro
            _j = _i
    return _dentro
...
def _ibge_malha_aneis(cod_ibge):
    """[IBGE-MALHAS - 108ª geração] Baixa o polígono oficial do município (API v3 de malhas do IBGE,
    GeoJSON leve) e devolve os anéis. Retorna [] se DESLIGADO ou em falha. Cacheado 24h. REQUER rede à
    API do IBGE (servicodados.ibge.gov.br)."""
    if not IBGE_MALHAS_ATIVO:
        return []
    try:
        import requests as _rq
        _u = (f"https://servicodados.ibge.gov.br/api/v3/malhas/municipios/{int(cod_ibge)}"
              f"?formato=application/vnd.geo+json&qualidade=intermediaria")
        _r = _rq.get(_u, timeout=8)
        _r.raise_for_status()
        _gj = _r.json()
        _aneis = []
        for _f in (_gj.get("features") or []):
            _aneis.extend(_extrair_aneis_geojson(_f.get("geometry", {})))
        if not _aneis and isinstance(_gj.get("geometry"), dict):
            _aneis = _extrair_aneis_geojson(_gj["geometry"])
        return _aneis
    except Exception as _e_m:
        logger.error(f"[IBGE-MALHAS] Falha ao obter malha do município {cod_ibge}: {_e_m}")
        return []


def _validar_ponto_no_municipio(lat, lon, cod_ibge, fornecedor_aneis=None):
    """[IBGE-MALHAS - 108ª geração] Verifica se (lat,lon) cai DENTRO do polígono oficial do município
    (Código IBGE). Retorna {disponivel, dentro, cod_ibge} ou {disponivel:False} se a malha não veio.
    `fornecedor_aneis` injetável para teste (default = _ibge_malha_aneis). PURO em relação ao fornecedor."""
    try:
        if float(lat) == 0.0 and float(lon) == 0.0:
            return {"disponivel": False}
    except (TypeError, ValueError):
        return {"disponivel": False}
    _fn = fornecedor_aneis if fornecedor_aneis is not None else (
        _ibge_malha_aneis if "_ibge_malha_aneis" in globals() else (lambda _c: []))
    try:
        _aneis = _fn(cod_ibge)
    except Exception:
        _aneis = []
    if not _aneis:
        return {"disponivel": False}
    return {"disponivel": True, "dentro": _ponto_em_poligono(lat, lon, _aneis), "cod_ibge": str(cod_ibge)}
            ...
            try:
                ...
                if _cols_ibge:
                    st.info(f"🏛️ **Código IBGE detectado** na(s) coluna(s): {', '.join(map(str, _cols_ibge))}. "
                            ...
                            "automaticamente pela base oficial (identificador prioritário, offline, O(1)).")
                ...
                if _col_cod_l and (_col_mun_l or _col_uf_l):
                    ...
                    if _confl_l:
                        with st.expander(f"⚠️ Validação IBGE: {len(_confl_l)} inconsistência(s) Código × Município/UF (amostra de até 500 linhas)", expanded=False):
...
with tab_alocacao:
    ...
    with col_a1: 
        file_dest = st.file_uploader("1. Planilha de Endereços / Entregas (Origens)", type=["xlsx"], key="up_dests_v19", help="Cada origem pode ser endereço, localidade, coordenadas ou o Código IBGE do município (7 dígitos). O resultado traz Cód IBGE Cliente/Origem.")
    with col_a2: 
        file_hubs = st.file_uploader("2. Planilha de Municípios / Bases (Destinos)", type=["xlsx"], key="up_hubs_v19", help="Cada base/hub pode ser identificado por nome, coordenadas ou Código IBGE do município (7 dígitos). O resultado traz Cód IBGE Hub.")
...
with tab_proximidade:
    ...
    if not _opcoes_mun:
        ...
        _cbusca1, _cbuscaC, _cbusca2 = st.columns([48, 27, 25])
        with _cbusca1:
            _mun_sel = st.selectbox("📍 Município de Origem (nome — digite para buscar)",
                                    options=_opcoes_mun, index=None, placeholder="Ex.: Ribeirão Cascalheira - MT",
                                    key="prox_mun_sel")
        with _cbuscaC:
            _cod_prox = st.text_input("🔢 ou Código IBGE", key="prox_cod", placeholder="Ex.: 3550308",
                                      help="Informe o Código IBGE (7 dígitos) — resolve o município oficial e define a origem.")
        with _cbusca2:
            _n_viz = st.number_input("Quantos vizinhos", min_value=3, max_value=15, value=5, step=1, key="prox_n")
        # [IBGE-INPUT - 102ª geração] Seleção da origem por Código IBGE: se informado e válido, resolve o
        # município oficial e SOBREPÕE a seleção por nome (localiza a opção exata na lista de busca para
        # manter toda a análise a jusante inalterada).
        _cod_prox_norm = _e_codigo_ibge(_cod_prox)
        if _cod_prox_norm:
            _res_prox = _resolver_por_codigo_ibge(_cod_prox_norm)
            if _res_prox:
                _alvo_mun = unidecode(str(_res_prox['municipio'])).upper().strip()
                _alvo_uf = str(_res_prox['uf']).upper().strip()
                _match_prox = next((o for o in _opcoes_mun
                                    if unidecode(str(o)).upper().strip().endswith(f"- {_alvo_uf}")
                                    and unidecode(str(o)).upper().split(" - ")[0].strip() == _alvo_mun), None)
                if _match_prox:
                    _mun_sel = _match_prox
                    st.caption(f"🏛️ Código {_cod_prox_norm} → **{_match_prox}** (origem definida pelo Código IBGE).")
                else:
                    st.caption(f"🏛️ Código {_cod_prox_norm} → {str(_res_prox['municipio']).title()}/{_res_prox['uf']} "
                               "(município não encontrado na lista de busca — selecione pelo nome).")
            else:
                st.caption(f"⚠️ Código {_cod_prox_norm} não encontrado na base oficial do IBGE.")
        ...
        if st.session_state.get('prox_resultado'):
            ...
            # quando ativa: ranking por ESTRADA entre sedes municipais, instantâneo e sem consumir APIs.
            # GATED pela flag + base disponível — não aparece se o DistBrasil não estiver configurado.
...
# ============================================================================
_HANDBOOK_HTML_B64 = (
    ""
)
